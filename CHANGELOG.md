...
- 修复 Windows GUI 向 Rust MCP 客户端返回超过命名管道缓冲区的实时快照时，错误混用 `PIPE_NOWAIT` 与 overlapped I/O，导致响应在 500ms 后被关闭并误报“小狸离线”的问题。
- 响应写回恢复为独立、并发受限 worker 内的完整阻塞写；停读客户端只占用自身 worker，不会再阻塞监听器或其他已接入客户端。
- 新增生产 worker 的多缓冲区 Rust 往返回归，以及停读客户端与正常客户端的并发隔离回归。
